...
import './style.css';
import submitogg from './submit.ogg';

//const apiKey = 'sk-proj-0tU1J9KSjO2YHQXFFcx2T3BlbkFJLHNdJpEPYegD8g7h7I8n'; // Replace with your OpenAI API key
const apiUrl = '/api';
...
function typingEffectFunction(text: string) {
...
export async function generateAudio(feedbackText: string): Promise<string> {
  try {
    const response = await axios.post(`${apiUrl}/generateAudio`, { feedbackText });
...
export const generatePrompt = async (
  ...
  try {
    const response = await axios.post(`${apiUrl}/generatePrompt`, { messages, isSubmit, currentStep });
...
export const handleChatGPTResponse = async (
  ...
  try {
    ...
    console.log(prompt);
    const response = await axios.post(
      `${apiUrl}/handleChatGPTResponse`,
      { messages: prompt, isSubmit, currentStep, attempts },
    );
...
export async function handleButtonClick(
  ...
  if (submitState.currentStep === 0) {
    ...
    if (isCorrect) {
      submitState.currentStep++;  // Move to the next step only if the answer is correct
      submitButton!.textContent = 'Next'; // Update button text for next step
      console.log('This is happening');
    } else if (submitState.attempts === 2) {  // If it's the second attempt and the answer is still incorrect
    ...
    try {
      const userResponse = await handleNEOAIButtonClick(); // Await the user response
      chatHistory.push({
        role: 'user',
        content: `User's response: ${userResponse}`,
      });
